...
export async function googleSearchImages(input: string): Promise<{
  success: boolean;
  data: GoogleSearchImage[];
  error: string | null;
}> {
  try {
    if (!process.env.GOOGLE_SEARCH_API_KEY) {
    ...
    const searchItems = response.data.items.map((item: GoogleSearchImage) => ({
      title: item.title,
      link: item.link,
      displayLink: item.displayLink,
    })) as GoogleSearchImage[];
    return {
      success: true,
      data: searchItems,
      error: null,
